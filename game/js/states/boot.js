var bootState = {
  ...
  create: function () {
      ...
      $.ajax({
        url: "http://localhost:8000/api/user?token="+token,
        success: function(result) {
          if(result.success === false) {
              window.location.replace("login.php");
          } else {
          $.ajax({
            url: "http://localhost:8000/api/character?token="+token,
  ...
  getArea: function(id) {
    return $.ajax({
      url: "http://localhost:8000/api/area/id/"+id
    });
  },

  getMonsters: function(token) {
  return $.ajax({
      url: "http://localhost:8000/api/character/monsters?token="+token,
    });
  },
