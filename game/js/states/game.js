var explorationMode={
      ...
      explorationMode.portalGroup.forEach(function(child) {
        if(child.index == item.index && child.xPos == item.x && child.yPos == item.y) {
          ...
          $.ajax({
            ...
            url: "http://37.123.188.101:8000/api/area/id/"+child.tpAreaId,
  ...
  getCharactersByType: function(types){
    game.paused = true;
    return $.ajax({
      url: "http://37.123.188.101:8000/api/monster/type/" + types
    });
  },
  ...
  getMonsterAttacksById: function(monster) {
    return $.ajax({
      url: "http://37.123.188.101:8000/api/monster/" + JSON.stringify([monster.id]) + "/attacks"
    });
  },
  ...
  getAttacksById: function(monster, attacks) {
    ...
    if(arr.length > 0) {
      ...
      $.ajax({
        url: "http://37.123.188.101:8000/api/attack/" + attackIds,
